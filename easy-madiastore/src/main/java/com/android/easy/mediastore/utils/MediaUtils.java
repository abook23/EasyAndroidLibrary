...
public class MediaUtils {
    ...
    public static Uri createImagePathUri(final Context context, String fileName) {
        ...
        values.put(MediaStore.Images.Media.MIME_TYPE, MediaStoreConfig.MIME_TYPE_IMAGE);
    ...
    public static Uri createImageVideoUri(final Context context, String fileName) {
        ...
        values.put(MediaStore.Images.Media.DISPLAY_NAME, imageName);
        values.put(MediaStore.Images.Media.DATE_TAKEN, time);
        values.put(MediaStore.Images.Media.MIME_TYPE, MediaStoreConfig.MIME_TYPE_VIDEO);
        // 判断是否有SD卡,优先使用SD卡存储,当没有SD卡时使用手机存储
        if (status.equals(Environment.MEDIA_MOUNTED)) {
            imageFilePath[0] = context.getContentResolver()
                    .insert(MediaStore.Files.getContentUri("external"), values);
    ...
    public static int[] getLocalVideoWidthOrHeight(String videoPath) {
        ...
        try {
            ...
            wh[0] = Integer.parseInt(mmr.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_WIDTH));
            wh[1] = Integer.parseInt(mmr.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_HEIGHT));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return wh;
