...
//    private String mSdRootPath = Environment.getExternalStorageDirectory().getAbsolutePath();
    ...
    private Bitmap loadBitmap(String uri, int width, int height, int inSampleSize, boolean isRound) {
        ...
        if (bitmap == null) {
            if (uri.startsWith("http://") || uri.startsWith("https://")) {
                ...
                if (width > 0 && height > 0) {
                    bitmap = BitmapUtils.getSmallBitmap(uri, width, height);
                } else if (inSampleSize == 0) {
                    bitmap = BitmapUtils.getSmallBitmap(uri, mContext);
                } else if (inSampleSize > 1) {
                    bitmap = BitmapUtils.getSmallBitmap(uri, inSampleSize);
                }
            ...
            if (bitmap != null) {
                try {
                    if (isRound) {
                        Bitmap bm_r = BitmapUtils.toRoundBitmap(bitmap);
                        bitmap = bm_r;
                        bm_r.recycle();
                    }
    ...
    private Bitmap loadBitmapForHttp(String url, int width, int height) {
        ...
        if (file == null)
            return null;
        return BitmapUtils.getSmallBitmap(file.getPath(), width, height);
    }
    ...
    private Bitmap loadBitmapForHttp(String url, int inSampleSize) {
        ...
        if (file == null)
            return null;
        return BitmapUtils.getSmallBitmap(file.getPath(), inSampleSize);
    }
